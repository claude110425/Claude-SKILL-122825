PreviewAPIUSER GUIDE: FDA 510(k) Agentic Reviewer
...
Five Detailed Step-by-Step Examples
...
Click "Upload files"
Select the SKILL.md file from your computer
Wait for upload to complete (green checkmark)
The file will appear in "Project Knowledge"

Step 4: Start a New Conversation

Click "Start new chat" (or press Ctrl+N / Cmd+N)
You're now ready to use the skills!
...
In "Instructions" box, paste the entire SKILL.md content
Under "Knowledge", upload any reference documents
Click "Save" → "Only me" (or share as needed)
...
Type: "I've uploaded your instruction manual. Please acknowledge that you understand all 11 skills."
...
In the top area, you'll see "System instructions" section
Click to expand it
Paste the System Prompt Section from SKILL.md
...
On the right side, find "Attach files" option
Click and upload your documents
Or paste text directly into the chat
...
Sign in (optional, but recommended for history)

Step 2: Set Context
Since Perplexity doesn't have persistent file upload:

Start your prompt with context-setting
Include relevant excerpts from SKILL.md in your prompt

Example Prompt Structure:
CopyContext: I'm using an FDA 510(k) review system with 11 specialized skills:
...
Go to: https://x.com (Twitter/X)
Sign in to your X account
Look for Grok icon (usually bottom right or in sidebar)
Click to open Grok chat

Step 2: Set Up Context

Start a new conversation
Paste system instructions from SKILL.md
...
CopyDevice Name: AcuGluco Pro CGM
...
CopyI need to use Skill 2: Intelligent 510(k) Report Generator.
...
Claude will process for 30-60 seconds
You'll see a comprehensive report being generated in real-time
...
markdownCopy| Attribute | Details |
|-----------|---------|
...
Click the "Copy" button that appears at the top of Claude's response
...
Save with .md extension (e.g., "K242567_Review_Report.md")
Use a markdown viewer or converter

Method C: Claude's Built-in Export

Click the three dots (⋯) at the top right of the response
Select "Copy as Markdown"
Paste into your preferred document
...
Solution:
CopyThe report you generated is only [X] words. Please expand it to meet 
...
Solution:
CopyThe report is missing some required tables. Please add:
...
Solution:
CopyPlease regenerate the report with additional emphasis on:
...
Start a new chat
If you created a custom GPT, select "FDA 510(k) Reviewer"


Step 3: Upload or Paste Documents
Type this prompt:
CopyI need to use Skill 3: Dual-Version Document Comparator.
...
markdownCopy# Document Comparison Report
...
Identify what changed (127 differences documented)
Prioritize review focus (38 HIGH significance items)
Assess AI adequacy (AI request appears addressed)
Create follow-up questions (3 remaining minor items)
Update review timeline (sponsor responded adequately, can proceed)


Step 6: Export and Use
For Review Documentation:
CopyCopy the "HIGH Significance Changes" section and paste into your 
review memo under "Response to Additional Information Request"
For Team Discussion:
CopyCopy the Summary Statistics and Assessment sections for team meeting
For Sponsor Communication:
CopyCopy the "Remaining Minor Questions" if you need to send a follow-up 
email or phone call discussion points

Common Issues and Solutions
Issue 1: Fewer than 100 differences found
Solution:
CopyYou identified only [X] differences, but I need at least 100. Please 
...
Solution:
CopyPlease add the "Regulatory Significance" column to your table with 
...
Solution:
CopyPlease create a separate summary table showing ONLY the HIGH 
...
Copy510(k) REVIEW TEAM MEETING MINUTES
...
Type this prompt:
CopyI need to use Magic Tool 3: Action Item Extraction & Task List Generator.
...
Copy the "Summary Table" section
Paste into Excel, Google Sheets, or project management tool (Asana, Trello, Monday)
Track progress as items are completed

For Team Distribution:

Copy the relevant section for each team member

Example: Send Dr. Brown items 001, 008, 010-013, 016-017


Email or share via collaboration tool

For Personal To-Do:

Copy the "Quick Copy Checklist" section
Paste into your personal task app (Todoist, Notion, Apple Reminders)
Check off items as you complete them

For Meeting Preparation:

Copy the "Timeline Visualization" section
Include in meeting agenda or presentation slides
Use to communicate project status to leadership


Common Issues and Solutions
Issue 1: Want to add custom fields (like "Review Section" or "Document Location")
Solution:
CopyPlease add these additional columns to the action item table:
...
Solution:
CopyPlease reformat the action item list as a CSV (comma-separated values) file 
...
Solution:
CopyPlease create separate action item lists for each assignee:
...
Solution:
CopyI think there are additional implicit action items you may have missed. 
